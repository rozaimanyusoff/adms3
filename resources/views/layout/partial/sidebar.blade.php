        ...
        <div class="menu">
            @php
				...
				foreach (config('sidebar.menu') as $key => $menu) {
					...
					$hasSub = (!empty($menu['children'])) ? 'has-sub' : '';
					$menuUrl = (!empty($menu['url'])) ? $menu['url'] : '';
					$menuLabel = (!empty($menu['label'])) ? '<span class="badge bg-danger ms-1">'. $menu['label'] .'</span>' : '';
					//$menuIcon = (!empty($menu['icon'])) ? '<span class="menu-icon"><i class="'. $menu['icon'] .'"></i>'. $menuLabel .'</span>' : '';
					$menuIcon = (!empty($menu['icon'])) ? '<span class="menu-icon"><i class="'. $menu['icon'] .'"></i></span>' : '';
					$menuText = (!empty($menu['text'])) ? '<span class="menu-text">'. $menu['text'] . $menuLabel.'</span>' : '';
					$menuCaret = (!empty($hasSub)) ? '<span class="menu-caret"><b class="caret"></b></span>' : '';
					$menuSubMenu = '';
